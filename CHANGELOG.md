...
- The post-setup panel is trimmed to the hand-off: after the first run it shows only the prompt to give your agent (with a "copied to your clipboard" confirmation), nothing else. The "Created" file tree now prints during scaffolding (right under `Created <dir>`) instead of in the final panel, and the docs link is gone from the panels. The bundled workspace `README.md` is now a few lines so it doesn't compete with the guided flow.
- The agent picker spells out what choosing an agent does (set it up *and* launch it), and labels `codex` as `codex (or other agents, e.g. Copilot)` since its `.agents/` layout is a cross-agent convention.
