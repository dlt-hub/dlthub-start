...
def _(dlt):
    ...
    """
    from pipeline import sample_shop
    pipeline_report = dlt.pipeline(
        pipeline_name="sample_shop_report",
        destination="warehouse",
        dataset_name="sample_shop",
    )
    load_info = pipeline_report.run(sample_shop().add_limit(1))
    return


@app.cell
def _(dlt):
    pipeline = dlt.attach("sample_shop_report")
