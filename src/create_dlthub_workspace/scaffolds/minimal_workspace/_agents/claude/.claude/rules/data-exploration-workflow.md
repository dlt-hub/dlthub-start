...
- From **one-shot** (after `create-minimal-pipeline` and `deploy-minimal-pipeline` both complete) — pipeline name and dataset are already known. `explore-data` should skip `list_pipelines` discovery and go straight to `list_tables`.
